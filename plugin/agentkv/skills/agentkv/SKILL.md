...
- **Account-key**: for a *managed* wallet that can't sign (e.g. [awal](https://www.npmjs.com/package/awal)), an opaque `ak_…` **bearer token** is the identity + namespace and a **local** encryption key encrypts — decoupled from the paying wallet, so any signing wallet can fund the account.
