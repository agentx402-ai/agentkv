...
<details>
<summary>From a local checkout (before <code>@agentkv/cli</code> is published)</summary>
...
- The marketplace entry lives at the **repo root**:
  [`.claude-plugin/marketplace.json`](../.claude-plugin/marketplace.json) (required there for
  `/plugin marketplace add` to find it).
